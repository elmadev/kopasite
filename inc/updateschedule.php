...
//$result = mysql_query("SELECT level.LevelIndex, level.LevelPackIndex, levelpack.LevelPackCategory FROM level, levelpack WHERE level.LevelPackIndex = levelpack.LevelPackIndex"); // recalc all
		...
		if($row['LevelIndex'] != 0){
			...
			foreach($pojos_change as $k => $p){
				if($row['LevelPackCategory'] == "hoyla"){
					$kp = $p / 2;
					mysql_query("UPDATE kuski SET Kinglist = Kinglist+".$kp." WHERE KuskiIndex = '".$k."'");
				}elseif($row['LevelPackCategory'] == "Moposite"){
					$kp = 0;
				}else{
					$kp = $p;
					mysql_query("UPDATE kuski SET Kinglist = Kinglist+".$kp." WHERE KuskiIndex = '".$k."'");
